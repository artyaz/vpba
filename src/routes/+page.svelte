<script lang="ts">
	...
	import { getWords, bumpRecollection } from '$lib/db.client';
	...
	function submit() {
		...
		const delta = ok ? 1 : -1;
		const target = items.find((i) => i.id === current!.id);
		if (target) target.recollection = Math.max(0, target.recollection + delta);
		if (dbReady()) {
			bumpRecollection(settings.dbUrl, settings.dbTable, current.id, delta).catch(() => {});
		}
	}
...
{:else if !current}
	<div class="empty">
		<p class="big">No sentences to practice yet.</p>
		<p class="muted">
			{dbReady() ? 'Add a few words and come back.' : 'Connect your database in settings.'}
		</p>
...
{:else}
	<section class="card">
		...
		<button class="hint" onclick={() => (showHint = !showHint)}>
		...
		{#if azureReady() && status !== 'asking'}
			<div class="say"><Pronounce word={current.headword} /></div>
		{/if}
	</section>
{/if}
